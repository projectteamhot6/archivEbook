...
EPUBJS.Reader = function(bookPath, _options) {
	...
	this.localStorageSetting();					// 수정한 부분입니다.
...
//수정한 부분입니다.
var information;
EPUBJS.Reader.prototype.localStorageSetting = function(){
	$.ajax({
		url : 'getStorage'
		, type : 'get'
		, data : 'json'
	...
	localStorage.setItem(this.settings.bookKey,information);
}
EPUBJS.Reader.prototype.success = function(e){
	if(e != null && e != ""){
		information = JSON.stringify(e);
	};
}
...
EPUBJS.reader.BookmarksController = function() {
	...
	var createBookmarkItem = function(cfi) {
		...
		link.addEventListener("click", function(event){
				...
				//rendition.display(cfi);	수정	
				location.href = "read#"+cfi;
...
EPUBJS.reader.NotesController = function() {
	...
	var insertAtPoint = function(e) {
		...
		*/
				annotation = {
					...
					anchor: page_start,				//수정 원래 page_start x // cfi
					body: $text.val()
				}
				...
				//placeMarker(annotation);	//원래 주석 아니었음
	...
	var addAnnotation = function(annotation){
		...
		link.onclick = function(){
			//rendition.display(annotation.anchor);	수정
			location.href = "read"+link.href;
			return false;
		};
...
var page_start;		//수정한 부분입니다.
var page_end;		//수정한 부분입니다.
EPUBJS.reader.ReaderController = function(book) {
	...
	rendition.on('relocated', function(location){
		page_start = location.start.cfi;		//수정
		page_end = location.end.cfi;			//수정
		if (location.atStart) {
