package global.sesoc.archive.util;
...
public class FileService {
	...
	public static String saveFile(MultipartFile mfile, String uploadPath) {
		...
		SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd");  //오라클로치면 to_char 
		String savedFilename = sdf.format(new Date());
		
		//원본 파일의 확장자
		String ext;
		int lastIndex = originalFilename.lastIndexOf('.');			//마지막으로 .나오는 뒷부분 hwp 를 읽어낸거다.
		//확장자가 없는 경우
		if (lastIndex == -1) {
			ext = "";
		}
		...
		while (true) {
			...
			//같은 이름의 파일이 있으면 이름 뒤에 long 타입의 시간정보를 한번 더 덧붙임.
			savedFilename = savedFilename + new Date().getTime();	 //기준시간 으로 계속 업로드 되게. 
		}		
		
		//파일 저장 
		try {
			mfile.transferTo(serverFile);		//파일 복사
		} catch (Exception e) {
			savedFilename = null;
			e.printStackTrace();
		}
